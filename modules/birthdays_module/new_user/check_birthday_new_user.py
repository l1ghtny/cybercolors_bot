...
async def check_birthday_new_user(client, user, interaction):
    conn, cursor = await basevariables.access_db_regular()
    query = 'SELECT * from "public".users as users inner join "public".servers as servers using(server_id) where ' \
            'user_id=%s and server_id=%s'
    values = (user.id, interaction.guild_id)
