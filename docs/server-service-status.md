...
The dashboard requests status every 60 seconds, including in background tabs while the browser permits execution, and checks cached report expiry every second. Tabs for the same signed-in user, API origin and server share a cached response under a Web Lock. The cache is reusable for less than 60 seconds from request start; followers schedule their next check against that original request, rather than extending the interval. No authentication credentials are stored. Clock changes, resume checks and stale cache entries force a new request. Missing browser coordination/storage or a lock held by a suspended tab fall back to independent polling. Requests are aborted after ten seconds so a stalled connection cannot prevent later polls. Browser suspension can pause timers; returning to the tab triggers a fresh request. It compares timestamps against `server_time` plus monotonic elapsed time from request start, conservatively counting network time against the remaining report lifetime. An incorrect device clock or timezone does not affect freshness. On focus or returning to a visible tab, cached health is invalidated until a new request succeeds; this covers browsers whose monotonic clock pauses during device sleep. A failed request immediately yields unknown, including when React Query retains old successful data. Switching servers uses a separate cache key. The indicator and conditional banner reflect current observations directly; no delayed recovery or historical coverage claim is made.
