...
async def _release_notes_scenario() -> None:
    ...
    assert manifest.releases[0].id == "2026-09-16-overview-birthday-channel-name"
    assert manifest.releases[0].change_type == "fixed"
    assert manifest.releases[0].surface == "dashboard"
    assert manifest.releases[0].title.en == "Birthday channel shown by name"
    assert manifest.releases[0].title.ru == "Название канала поздравлений в обзоре сервера"
