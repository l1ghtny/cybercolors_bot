...
async def _release_notes_scenario() -> None:
    ...
    assert len(manifest.releases) == 60
    assert manifest.releases[0].id == "2026-09-15-discord-command-sync-recovery"
    assert manifest.releases[0].change_type == "fixed"
    assert manifest.releases[0].surface == "bot"
    assert manifest.releases[1].id == "2026-09-06-youtube-duration-and-indexing"
    assert manifest.releases[2].id == "2026-09-04-youtube-audio-download-compatibility"
    assert all(release.id != "2026-09-04-knowledge-discord-identities" for release in manifest.releases)
    assert manifest.releases[3].id == "2026-09-03-readable-member-profile-layout"
    assert manifest.releases[3].title.en == "Member profiles stay readable in narrower windows"
    assert manifest.releases[3].title.ru == "Профили участников удобно читать даже в узких окнах"
