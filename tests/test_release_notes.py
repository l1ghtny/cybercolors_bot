...
async def _release_notes_scenario() -> None:
    ...
    assert manifest.releases[0].title.en == "Background tasks recover after temporary failures"
    assert manifest.releases[0].title.ru == "Фоновые задачи восстанавливаются после временных сбоев"
    assert manifest.releases[1].id == "2026-09-18-monitoring-log-reasons"
    assert manifest.releases[1].change_type == "fixed"
    assert manifest.releases[1].surface == "both"
    assert manifest.releases[1].title.en == "Correct reasons and actors in monitoring history"
    assert manifest.releases[1].title.ru == "Исправлены причины и авторы событий наблюдения"
    assert manifest.releases[2].id == "2026-09-16-service-status-device-clock"
    assert manifest.releases[2].change_type == "fixed"
    assert manifest.releases[2].surface == "dashboard"
    assert manifest.releases[3].id == "2026-09-16-overview-birthday-channel-name"
    assert manifest.releases[3].change_type == "fixed"
    assert manifest.releases[3].surface == "dashboard"
    assert manifest.releases[3].title.en == "Birthday channel shown by name"
    assert manifest.releases[3].title.ru == "Название канала поздравлений в обзоре сервера"
    assert manifest.releases[4].id == "2026-09-16-server-service-status"
    assert manifest.releases[4].change_type == "added"
    assert manifest.releases[4].surface == "dashboard"
    assert manifest.releases[5].id == "2026-09-15-discord-command-sync-recovery"
    assert manifest.releases[5].change_type == "fixed"
    assert manifest.releases[5].surface == "bot"
    assert manifest.releases[6].id == "2026-09-06-youtube-duration-and-indexing"
    assert manifest.releases[7].id == "2026-09-04-youtube-audio-download-compatibility"
    assert all(release.id != "2026-09-04-knowledge-discord-identities" for release in manifest.releases)
    assert manifest.releases[8].id == "2026-09-03-readable-member-profile-layout"
    assert manifest.releases[8].title.en == "Member profiles stay readable in narrower windows"
    assert manifest.releases[8].title.ru == "Профили участников удобно читать даже в узких окнах"
