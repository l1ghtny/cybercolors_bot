...
async def _release_notes_scenario() -> None:
    ...
    assert manifest.releases[0].id == "2026-09-18-monitoring-log-reasons"
    assert manifest.releases[0].change_type == "fixed"
    assert manifest.releases[0].surface == "both"
    assert manifest.releases[0].title.en == "Correct reasons and actors in monitoring history"
    assert manifest.releases[0].title.ru == "Исправлены причины и авторы событий наблюдения"
